...
{
	HAL_Delay(20);  // 等待模块上电稳定，最少5ms，然后拉低PWDN  	
	
	OV5640_PWDN_OFF;  // PWDN 引脚输出低电平，不开启掉电模式，摄像头正常工作，此时摄像头模块的白色LED会点亮
  HAL_Delay(20);    
	DCMI_RST_H
// 根据OV5640的上电时序，PWDN拉低之后，要等待1ms再去拉高RESET，奋斗的OV5640模块采用硬件RC复位，也具有IO口线复位, 持续时间大概在6~10ms，
...
void OV5640_Config(void)
{
	...
	//OV5640_Set_Vertical_Flip(1);   //垂直反转
	//OV5640_Set_Horizontal_Mirror(1);  //水平反转
}
